...
L1_loss <- function(y, yhat)
  (y-yhat)^2+2*abs(y-yhat)

tilted_loss <- function(y, yhat, tau){
  ifelse(y-yhat>0, tau * ((y-yhat)^2+2*abs(y-yhat)), 2 *tau * ((y-yhat)^2+2*abs(y-yhat)))
}
```
...
lines(x = x_grid, y = lin_pred_tau25, col = 'blue', lwd = 2)
...
lines(x = x_grid, y = nonlin_pred_tau25, col = 'blue', lwd = 2)
lines(x = x_grid, y = nonlin_pred_tau75, col = 'pink', lty=2,lwd = 2)
legend("topleft", c("L1 loss", " tau = 0.25", " tau = 0.75"), lwd = c(2,2,2),col = c("darkgreen", "pink", "yellow"))
