...
fit.knn <- nnet(x=dat$x, y=dat$y, size=10, entropy=TRUE, decay=0.02)
pred.knn<- predict(fit.knn , dat$xnew, type="class")
pred1.knn <- matrix(pred.knn, length(dat$px1), length(dat$px2))
```
...
knn.boundary <- contourLines(dat$px1, dat$px2, pred1.knn, levels=0.5)
sapply(knn.boundary, lines,col='green')
